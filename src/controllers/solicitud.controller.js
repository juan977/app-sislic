...
solicitudCtr.insertSolicitud = async (req, res) => {
    try {
        const { id_contribuyente, id_establecimiento, tipotramite, notif_direccion, notif_distrito, correo, telefono, anexo_02, declaracion_jurada,
            codigo_solicitud, id_solestado } = req.body;
...
solicitudCtr.obtener_lista_solicitudes_pendientes_validados = async (req, res) => {
    try {
        const response = await pool.query(
            `select 
...
solicitudCtr.obtener_datos_colicitud_contribuyentes_establecimiento = async (req, res) => {
    try {
        const response = await pool.query(
            `select
                case when s.tipotramite = '1' then 'SOLO LIC. DE FUNCIONAMIENTO' end tipo_tramite,
...
solicitudCtr.modificar_actualizaciones_estado_solicitud = async (req, res) => {
    try {
...
//CU05
//Consultar estado
solicitudCtr.obtener_consultas_estado = async (req, res) => {
    try {
        const response = await pool.query(
...
solicitudCtr.obtener_datos_solicitud_contri_establecimiento = async (req, res) => {
    try {
        const response = await pool.query(
            `select
                case when s.tipotramite = '1' then 'SOLO LIC. DE FUNCIONAMIENTO' end tipo_tramite,
...
solicitudCtr.obtener_lista_solicitud_pendiente_evaluarRiesgo = async (req, res) => {
    try {
        const response = await pool.query(
            `select 
                case when s.tipotramite = '1' then 'SOLO LIC. DE FUNCIONAMIENTO' end tipo_tramite, 
